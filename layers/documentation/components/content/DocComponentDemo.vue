...
const hasAccInfo = computed(() =>
  Boolean(info.value.folder && info.value.file),
)
const demoPending = ref(hasAccInfo.value)
...
<template>
  <div class="border-muted-200 dark:border-muted-800 group mb-10 border-b">
    <div
      v-if="props.title || props.tag || (hasAccContent && props.dark)"
    ...
    >
      ...
      >
        <div v-if="'default' in $slots" :class="[hasAccContent && 'mb-10']">
          <div
        ...
        <div v-if="hasAccContent" class="flex flex-col gap-4">
          <div>
