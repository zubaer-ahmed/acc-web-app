    ...
    <div class="group-[&.scrolled]/landing:dark:bg-muted-950/95 group-[&.scrolled]/landing:border-muted-200 group-[&.scrolled]/landing:dark:border-muted-700 group-[&.scrolled]/landing:motion-safe:shadow-muted-300/30 group-[&.scrolled]/landing:motion-safe:dark:shadow-muted-800/20 flex items-center justify-between rounded-2xl border p-4 group-[&:not(.scrolled)]/landing:border-transparent group-[&.scrolled]/landing:bg-white/95 motion-safe:transition-all motion-safe:duration-200 group-[&.scrolled]/landing:motion-safe:shadow-xl">
      <div class="flex w-1/2 items-center gap-2 md:w-1/5">
        <NuxtLink to="/" class="ms-2 inline-flex" aria-label="Go to Tairo homepage">
          <TairoLogoText class="text-primary-500 group-[&.scrolled]/landing:h-6 group-[&:not(.scrolled)]/landing:h-7 motion-safe:transition-all motion-safe:duration-200" />
        </NuxtLink>
      </div>
      <div
      ...
      >
        <NuxtLink to="/feature" class="inline-flex items-center justify-center px-6"> Feature </NuxtLink>
        <NuxtLink to="/financial-model" class="inline-flex items-center justify-center px-6"> Financial Model </NuxtLink>
        <NuxtLink to="/marketplace" class="inline-flex items-center justify-center px-6"> Marketplace </NuxtLink>
        <NuxtLink to="/ecommerce" class="inline-flex items-center justify-center px-6"> ECommerce </NuxtLink>
        <div class="px-6">
          <button type="button" class="group-[&.scrolled]/landing:bg-muted-100 group-[&.scrolled]/landing:dark:bg-muted-900 group-[&.scrolled]/landing:border-muted-100 group-[&.scrolled]/landing:dark:border-muted-800 group-[&.scrolled]/landing:text-muted-400 group-[&.scrolled]/landing:dark:text-muted-500 group-[&.scrolled]/landing:hover:text-primary-500 group-[&.scrolled]/landing:dark:hover:text-primary-500 group-[&:not(.scrolled)]/landing:text-muted-800 group-[&:not(.scrolled)]/landing:dark:text-muted-200 group flex items-center gap-2 rounded-xl py-1 pe-1 ps-3 group-[&.scrolled]/landing:border" @click="isSearchOpen = true" aria-label="Open search">
            <Icon name="lucide:search" class="h-4 w-4 motion-safe:transition-colors motion-safe:duration-300" />
            <span class="group-[&.scrolled]/landing:dark:bg-muted-800 group-[&.scrolled]/landing:border-muted-200 group-[&.scrolled]/landing:dark:border-muted-700 group-[&.scrolled]/landing:group-hover:text-muted-600 group-[&.scrolled]/landing:dark:group-hover:text-muted-100 rounded-lg border px-2 py-0.5 group-[&:not(.scrolled)]/landing:border-transparent group-[&.scrolled]/landing:bg-white group-[&.scrolled]/landing:shadow motion-safe:transition-colors motion-safe:duration-300">
      ...
      <div class="flex w-1/2 items-center justify-end gap-4 md:w-1/5">
        ...
        <BaseButton shape="curved" color="primary" to="/auth/signin" class="ltablet:!flex !hidden lg:!flex"> Signin </BaseButton>
        <button type="button" class="ltablet:hidden flex items-center justify-center lg:hidden" @click="isMobileOpen = !isMobileOpen" :aria-label="isMobileOpen ? 'Close menu' : 'Open menu'">
          <div class="space-y-1.5">
            <span class="bg-primary-600 block h-0.5 motion-safe:transition-all motion-safe:duration-300" :class="isMobileOpen ? 'w-2' : 'w-6'"></span>
            <span class="bg-primary-600 block h-0.5 motion-safe:transition-all motion-safe:duration-300" :class="isMobileOpen ? 'w-6' : 'w-6'"></span>
            <span class="bg-primary-600 block h-0.5 motion-safe:transition-all motion-safe:duration-300" :class="isMobileOpen ? 'w-4' : 'w-6'"></span>
          </div>
        </button>
      </div>
    </div>
