...
import { useSidebarBusiness } from '../composables/sidebar-business'
console.log('i am in business sidebar-business')
...
const { setup, currentName, isOpen, toggle } = useSidebarBusiness()
setup()
...
const sidebarEnabled = computed(() => {
  return (app.tairo.sidebar?.business?.enabled !== false && props.sidebar)
})
const toolbarEnabled = computed(() => {
  return (app.tairo.sidebar?.toolbar?.enabled !== false && props.toolbar)
})
const circularMenuEnabled = computed(() => {
  return (app.tairo.sidebar?.circularMenu?.enabled !== false && props.circularMenu)
})
...
<template>
  <div class="bg-muted-100 dark:bg-muted-900 pb-20">
    <slot name="sidebar">
      ...
      >
        <div
          v-if="app.tairo.sidebar?.business?.logo?.component"
              ...
              <component
                :is="
                  resolveComponentOrNative(
                    app.tairo.sidebar?.business.logo.component,
                  )
                "
                v-bind="app.tairo.sidebar?.business.logo.props"
              ></component>
            </NuxtLink>
          </slot>
        </div>
