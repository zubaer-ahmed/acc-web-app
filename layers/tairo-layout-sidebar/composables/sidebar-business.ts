 ...
 * ```
 */
export function useSidebarBusiness() {
  const app = useAppConfig()
  const route = useRoute()

  const sidebars = computed(() => {
    if (
      app.tairo.sidebar?.business?.enabled === false ||
      app.tairo.sidebar?.business?.items?.length === 0
    ) {
      return []
    }
    return app.tairo.sidebar?.business?.items
  })
  ...
  function detect() {
    if (!app.tairo.sidebar?.business?.startOpen) {
      isOpen.value = false
      return
    }
