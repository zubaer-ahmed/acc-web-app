...
const VALIDATION_TEXT = {
  ...
  BANK_ACCOUNT_UID_REQUIRED: "Bank Account UID' is required.",
...
const zodSchema = z
    ...
    .superRefine((data, ctx) => {
      if (!data.bank_account_uid) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message: VALIDATION_TEXT.BANK_ACCOUNT_UID_REQUIRED,
          path: ['bank_account_uid'],
        })
      }
      ...
      if (!data.total_tax) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message: VALIDATION_TEXT.TOTAL_TAX_REQUIRED,
          path: ['total_tax'],
        })
      }
    })
...
const onSubmit = handleSubmit(
    async (values) => {
      success.value = false
      console.log('bank-transaction-create-success', values)
      try {
        await new Promise((resolve, reject) => {
          if (values.bank_account_uid === '') {
            setTimeout(
                () => reject(new Error('Bank Transaction validation error')),
                2000,
            )
          }
        ...
        if (error.message ==='Bank Transaction validation error') {
          setFieldError('bank_account_uid', 'This bank account uid code is not allowed')
    ...
    (error) => {
      success.value = false
      console.log('bank-transaction-create-error', error)
      document.documentElement.scrollTo({
        top: 0,
        behavior: 'smooth',
      })
      ...
      <div class="ltablet:max-w-full flex max-w-[425px] flex-col items-center gap-4 text-center md:flex-row md:text-left lg:max-w-full">
        <div>
          <BaseHeading as="h2" size="xl" weight="light" lead="tight" class="text-muted-800 dark:text-white">
            <span>New Bank Transaction</span>
          </BaseHeading>
          <BaseParagraph size="sm">
            <span class="text-muted-500"> Create a new  Bank Transaction record </span>
          </BaseParagraph>
        </div>
      </div>
      ...
      <form method="POST" action="" class="divide-muted-200 dark:divide-muted-700 grid divide-x sm:grid-cols-2" @submit.prevent="onSubmit">
        <div shape="curved" class="bg-muted-50 dark:bg-muted-800/60 space-y-8 p-10">
          <div class="mx-auto flex w-full max-w-[410px] flex-col">
            ...
            <div class="ltablet:col-span-8 col-span-12 lg:col-span-8">
              <Field v-slot="{ field, errorMessage, handleChange, handleBlur }" name="bank_account_uid">
                <BaseListbox
                    label="Bank Account"
                    :items="bankAccount"
                    :properties="{
                ...
                <div class="grid grid-cols-12 gap-4">
                  <div class="ltablet:col-span-6 col-span-12 lg:col-span-6">
                    <Field v-slot="{ field, errorMessage, handleChange, handleBlur }" name="type">
                      <BaseInput label="Type" placeholder="Ex: Acme Inc." :model-value="field.value" :error="errorMessage" :disabled="isSubmitting" type="text" @update:model-value="handleChange" @blur="handleBlur" />
                    </Field>
                  </div>
                  <div class="ltablet:col-span-6 col-span-12 lg:col-span-6">
                  ...
                  <div class="ltablet:col-span-6 col-span-12 lg:col-span-6">
                    <Field v-slot="{ field, errorMessage, handleChange, handleBlur }" name="total">
                      <BaseInput label="Total" placeholder="Ex: hello@acme.co" :model-value="field.value" :error="errorMessage" :disabled="isSubmitting" type="email" @update:model-value="handleChange" @blur="handleBlur" />
                    </Field>
                  </div>
                  <div class="ltablet:col-span-6 col-span-12 lg:col-span-6">
                    <Field v-slot="{ field, errorMessage, handleChange, handleBlur }" name="total_tax">
                      <BaseInput label="Total Tax" placeholder="Ex: hello@acme.co" :model-value="field.value" :error="errorMessage" :disabled="isSubmitting" type="email" @update:model-value="handleChange" @blur="handleBlur" />
                    </Field>
                  </div>
                </div>
              </div>
            ...
            <div class="grid grid-cols-12 gap-4">
              ...
              <div class="ltablet:col-span-6 col-span-12 lg:col-span-6">
                <Field v-slot="{ field, errorMessage, handleChange, handleBlur }" name="status">
                  <BaseInput label="Status" placeholder="Ex: hello@acme.co" :model-value="field.value" :error="errorMessage" :disabled="isSubmitting" type="email" @update:model-value="handleChange" @blur="handleBlur" />
                </Field>
              </div>
