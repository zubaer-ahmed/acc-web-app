...
const onSubmit = handleSubmit(
    async (values) => {
      success.value = false
      console.log('tax-exemption-create-success', values)
      try {
        await new Promise((resolve, reject) => {
          if (values.name === '') {
            setTimeout(
                () => reject(new Error('Tax Exemption validation error')),
                2000,
            )
          }
        ...
        if (error.message === 'Tax Exemption validation error') {
    ...
    (error) => {
      success.value = false
      console.log('tax-exemption-create-error', error)
      document.documentElement.scrollTo({
        top: 0,
        behavior: 'smooth',
      })
      ...
      <div class="ltablet:max-w-full flex max-w-[425px] flex-col items-center gap-4 text-center md:flex-row md:text-left lg:max-w-full">
        <div>
          <BaseHeading as="h2" size="xl" weight="light" lead="tight" class="text-muted-800 dark:text-white">
            <span>New Tax Exception</span>
          </BaseHeading>
          <BaseParagraph size="sm">
            <span class="text-muted-500"> Create a new  Tax Exception record </span>
          </BaseParagraph>
        </div>
      </div>
                ...
                <div class="grid grid-cols-12 gap-4">
                  ...
                  <div class="ltablet:col-span-6 col-span-12 lg:col-span-6">
                    <Field v-slot="{ field, errorMessage, handleChange, handleBlur }" name="code">
                      <BaseInput label="Code" placeholder="Ex: 123" :model-value="field.value"
            ...
            <div class="grid grid-cols-12 gap-4">
              <div class="ltablet:col-span-6 col-span-12 lg:col-span-12">
                <Field v-slot="{ field, errorMessage, handleChange, handleBlur }" name="reason">
                  <BaseInput label="Reason" placeholder="Ex: state level tax" :model-value="field.value"
                             :error="errorMessage" :disabled="isSubmitting" type="text"
                             @update:model-value="handleChange" @blur="handleBlur"/>
                </Field>
              </div>
