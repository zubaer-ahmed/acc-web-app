...
const VALIDATION_TEXT = {
  ...
  JURISDICTION_NAME_REQUIRED:"Jurisdiction Name is required.",
...
const zodSchema = z
    ...
    .superRefine((data, ctx) => {
      ...
      if (!data.jurisdiction_name) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message: VALIDATION_TEXT.JURISDICTION_NAME_REQUIRED,
...
const onSubmit = handleSubmit(
    async (values) => {
      success.value = false
      console.log('tax-authority-create-success', values)
      try {
        await new Promise((resolve, reject) => {
          if (values.name === '') {
            setTimeout(
                () => reject(new Error('Tax Authority validation error')),
                2000,
            )
          }
        ...
        if (error.message === 'Tax Authority validation error') {
    ...
    (error) => {
      success.value = false
      console.log('tax-authority-create-error', error)
      document.documentElement.scrollTo({
        top: 0,
        behavior: 'smooth',
      })
      ...
      <div class="ltablet:max-w-full flex max-w-[425px] flex-col items-center gap-4 text-center md:flex-row md:text-left lg:max-w-full">
        <div>
          <BaseHeading as="h2" size="xl" weight="light" lead="tight" class="text-muted-800 dark:text-white">
            <span>New Tax Authority</span>
          </BaseHeading>
          <BaseParagraph size="sm">
            <span class="text-muted-500"> Create a new  Tax Authority record </span>
          </BaseParagraph>
        </div>
      </div>
            ...
            <div>
              <div>
                <div class="grid grid-cols-12 gap-4">
                  ...
                  <div class="ltablet:col-span-6 col-span-12 lg:col-span-6">
                    <Field v-slot="{ field, errorMessage, handleChange, handleBlur }" name="jurisdiction_name">
                      <BaseListbox
                          label="Jurisdiction Namee"
                          :items="jurisdictionName"
                          :properties="{
                                        value: 'uid',
                                        label: 'name',
                                      }"
            ...
            <div class="grid grid-cols-12 gap-4">
              <div class="ltablet:col-span-6 col-span-12 lg:col-span-12">
                <Field v-slot="{ field, errorMessage, handleChange, handleBlur }" name="description">
                  <BaseInput label="Description" placeholder="Ex: state level tax" :model-value="field.value"
