...
const onSubmit = handleSubmit(
    async (values) => {
      success.value = false
      console.log('recurring-bill-create-success', values)
      try {
        await new Promise((resolve, reject) => {
          if (values.profile_name === '') {
            setTimeout(
                () => reject(new Error('Recurring bill validation error')),
                2000,
            )
          }
        ...
        if (error.message === 'Recurring bill validation error') {
    ...
    (error) => {
      success.value = false
      console.log('recurring-bill-create-error', error)
      document.documentElement.scrollTo({
        top: 0,
        behavior: 'smooth',
      })
      ...
      <div class="ltablet:max-w-full flex max-w-[425px] flex-col items-center gap-4 text-center md:flex-row md:text-left lg:max-w-full">
        <div>
          ...
          <BaseParagraph size="sm">
            <span class="text-muted-500"> Create a new  Recurring Bill record </span>
          </BaseParagraph>
        </div>
      </div>
