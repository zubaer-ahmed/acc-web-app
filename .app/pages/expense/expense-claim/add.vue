...
const onSubmit = handleSubmit(
    async (values) => {
      success.value = false
      console.log('expense-claim-create-success', values)
      try {
        await new Promise((resolve, reject) => {
          if (values.account_uid === '') {
            setTimeout(
                () => reject(new Error('Expense claim validation error')),
                2000,
            )
          }
        ...
        if (error.message === 'Expense claim validation error') {
    ...
    (error) => {
      success.value = false
      console.log('expense-claim-create-error', error)
      document.documentElement.scrollTo({
        top: 0,
        behavior: 'smooth',
      })
      ...
      <div class="ltablet:max-w-full flex max-w-[425px] flex-col items-center gap-4 text-center md:flex-row md:text-left lg:max-w-full">
        <div>
          ...
          <BaseParagraph size="sm">
            <span class="text-muted-500"> Create a new  Expense Claim record </span>
          </BaseParagraph>
        </div>
      </div>
            ...
            <div class="grid grid-cols-12 gap-4">
              ...
              <div class="ltablet:col-span-6 col-span-12 lg:col-span-12">
                <Field v-slot="{ field, errorMessage, handleChange, handleBlur }" name="payment_due_date">
                  <BaseInput label="Payment Due Date" placeholder="Ex: 2023-12-01" :model-value="field.value"
                             :error="errorMessage" :disabled="isSubmitting" type="text"
                             @update:model-value="handleChange" @blur="handleBlur"/>
                </Field>
              </div>
