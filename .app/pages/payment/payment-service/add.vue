...
const VALIDATION_TEXT = {
  ...
  PAY_NOW_TEXT_REQUIRED: "Pay Now Text is required.",
...
const onSubmit = handleSubmit(
    async (values) => {
      success.value = false
      console.log('payment-service-create-success', values)
      try {
        await new Promise((resolve, reject) => {
          if (values.name === '') {
            setTimeout(
                () => reject(new Error('Payment service validation error')),
                2000,
            )
          }
        ...
        if (error.message === 'Payment service validation error') {
    ...
    (error) => {
      success.value = false
      console.log('payment-service-create-error', error)
      document.documentElement.scrollTo({
        top: 0,
        behavior: 'smooth',
      })
      ...
      <div class="ltablet:max-w-full flex max-w-[425px] flex-col items-center gap-4 text-center md:flex-row md:text-left lg:max-w-full">
        <div>
          <BaseHeading as="h2" size="xl" weight="light" lead="tight" class="text-muted-800 dark:text-white">
            <span>New Payment Service</span>
          </BaseHeading>
          <BaseParagraph size="sm">
            <span class="text-muted-500"> Create a new  Payment Service record </span>
          </BaseParagraph>
        </div>
      </div>
          ...
          <div class="mx-auto w-full max-w-[410px]">
            <div class="grid grid-cols-12 gap-4">
              <div class="tablet:col-span-6 col-span-12 lg:col-span-12">
                <Field v-slot="{ field, errorMessage, handleChange, handleBlur }" name="url">
                  <BaseInput label="Url" placeholder="Ex: https://stripe.com" :model-value="field.value"
                             :error="errorMessage" :disabled="isSubmitting" type="text"
                             @update:model-value="handleChange" @blur="handleBlur"/>
                </Field>
              </div>
              <div class="tablet:col-span-6 col-span-12 lg:col-span-12">
                <Field v-slot="{ field, errorMessage, handleChange, handleBlur }" name="pay_now_text">
                  <BaseInput label="Pay Now Text" placeholder="Ex: thank you." :model-value="field.value"
                             :error="errorMessage" :disabled="isSubmitting" type="text"
                             @update:model-value="handleChange" @blur="handleBlur"/>
                </Field>
              </div>
