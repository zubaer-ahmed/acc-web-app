...
const VALIDATION_TEXT = {
  ...
  BILL_UID_REQUIRED: "Bill UID is required.",
  EXPENSE_UID_REQUIRED: "Expense UID is required.",
  EXPENSE_CLAIM_UID_REQUIRED: "Expense Claim UID is required.",
...
const zodSchema = z
    .object({
      ...
      bill_uid: z.string(),
      expense_uid: z.string(),
      expense_claim_uid: z.string(),
    ...
    .superRefine((data, ctx) => {
      ...
      if (!data.bill_uid) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message: VALIDATION_TEXT.BILL_UID_REQUIRED,
          path: ['bill_uid'],
        })
      }
      if (!data.expense_uid) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message: VALIDATION_TEXT.EXPENSE_UID_REQUIRED,
          path: ['expense_uid'],
        })
      }
      if (!data.expense_claim_uid) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message: VALIDATION_TEXT.EXPENSE_CLAIM_UID_REQUIRED,
          path: ['expense_claim_uid'],
        })
      }
...
const onSubmit = handleSubmit(
      ...
      try {
        await new Promise((resolve, reject) => {
          if (values.account_uid === '') {
            setTimeout(
                () => reject(new Error('Business payment validation error')),
                2000,
            )
          }
        ...
        if (error.message === 'Business payment validation error') {
          setFieldError('account_uid', 'This payment is not allowed')
    ...
    (error) => {
      success.value = false
      console.log('business-payment-create-error', error)
      document.documentElement.scrollTo({
        top: 0,
        behavior: 'smooth',
      })
      ...
      <div class="ltablet:max-w-full flex max-w-[425px] flex-col items-center gap-4 text-center md:flex-row md:text-left lg:max-w-full">
        <div>
          <BaseHeading as="h2" size="xl" weight="light" lead="tight" class="text-muted-800 dark:text-white">
            <span>New Business Payment</span>
          </BaseHeading>
          <BaseParagraph size="sm">
            <span class="text-muted-500"> Create a new  Business Payment record </span>
          </BaseParagraph>
        </div>
      </div>
            ...
            <div>
              <div>
                <div class="grid grid-cols-12 gap-4">
                  ...
                  <div class="ltablet:col-span-6 col-span-12 lg:col-span-6">
                    <Field v-slot="{ field, errorMessage, handleChange, handleBlur }" name="bill_uid">
                      <BaseListbox
                          label="Bill"
                          :items="bill"
                          :properties="{
                  ...
                  <div class="ltablet:col-span-6 col-span-12 lg:col-span-6">
                    <Field v-slot="{ field, errorMessage, handleChange, handleBlur }" name="expense_uid">
                      <BaseListbox
                          label="Expense"
                          :items="expense"
                          :properties="{
                  ...
                  <div class="ltablet:col-span-6 col-span-12 lg:col-span-6">
                    <Field v-slot="{ field, errorMessage, handleChange, handleBlur }" name="expense_claim_uid">
                      <BaseListbox
                          label="ExpenseClaim"
                          :items="expenseClaim"
                          :properties="{
                  ...
                  <div class="ltablet:col-span-6 col-span-12 lg:col-span-12">
                    <Field v-slot="{ field, errorMessage, handleChange, handleBlur }" name="payment_date">
                      <BaseInput label="Payment Date" placeholder="Ex: 2023-12-01" :model-value="field.value"
                                 :error="errorMessage" :disabled="isSubmitting" type="text"
                                 @update:model-value="handleChange" @blur="handleBlur"/>
                    </Field>
                  </div>
