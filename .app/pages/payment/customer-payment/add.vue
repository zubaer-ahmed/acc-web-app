...
const VALIDATION_TEXT = {
  ...
  CREDIT_NOTE_UID_REQUIRED: "Credit Note UID is required.",
...
const zodSchema = z
    ...
    .superRefine((data, ctx) => {
      ...
      if (!data.credit_note_uid) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message: VALIDATION_TEXT.CREDIT_NOTE_UID_REQUIRED,
          path: ['credit_note_uid'],
        })
      }
      if (!data.amount) {
        ctx.addIssue({
...
const onSubmit = handleSubmit(
    async (values) => {
      success.value = false
      console.log('customer-payment-create-success', values)
      try {
        await new Promise((resolve, reject) => {
          if (values.account_uid === '') {
            setTimeout(
                () => reject(new Error('Customer payment validation error')),
                2000,
            )
          }
        ...
        if (error.message === 'Customer payment validation error') {
    ...
    (error) => {
      success.value = false
      console.log('customer-payment-create-error', error)
      document.documentElement.scrollTo({
        top: 0,
        behavior: 'smooth',
      })
      ...
      <div class="ltablet:max-w-full flex max-w-[425px] flex-col items-center gap-4 text-center md:flex-row md:text-left lg:max-w-full">
        <div>
          ...
          <BaseParagraph size="sm">
            <span class="text-muted-500"> Create a new  Customer Payment record </span>
          </BaseParagraph>
        </div>
      </div>
            ...
            <div>
              <div>
                <div class="grid grid-cols-12 gap-4">
                  ...
                  <div class="ltablet:col-span-6 col-span-12 lg:col-span-6">
                    <Field v-slot="{ field, errorMessage, handleChange, handleBlur }" name="credit_note_uid">
                      <BaseListbox
                          label="CreditNote"
                          :items="creditNote"
                          :properties="{
                  ...
                  <div class="ltablet:col-span-6 col-span-12 lg:col-span-6">
                    <Field v-slot="{ field, errorMessage, handleChange, handleBlur }" name="payment_date">
                      <BaseInput label="Payment Date" placeholder="Ex: 2023-01-04" :model-value="field.value"
                                 :error="errorMessage" :disabled="isSubmitting" type="text"
                                 @update:model-value="handleChange" @blur="handleBlur"/>
                    </Field>
                  </div>
            ...
            <div class="grid grid-cols-12 gap-4">
              ...
              <div class="ltablet:col-span-6 col-span-12 lg:col-span-12">
                <Field v-slot="{ field, errorMessage, handleChange, handleBlur }" name="reference">
                  <BaseInput label="Reference" placeholder="Ex: " :model-value="field.value"
                             :error="errorMessage" :disabled="isSubmitting" type="text"
                             @update:model-value="handleChange" @blur="handleBlur"/>
                </Field>
              </div>
