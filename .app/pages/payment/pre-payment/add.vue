...
const onSubmit = handleSubmit(
    async (values) => {
      success.value = false
      console.log('per-payment-create-success', values)
      try {
        await new Promise((resolve, reject) => {
          if (values.account_uid === '') {
            setTimeout(
                () => reject(new Error('Pre payment validation error')),
                2000,
            )
          }
        ...
        if (error.message === 'Pre payment validation error') {
    ...
    (error) => {
      success.value = false
      console.log('pre-payment-create-error', error)
      document.documentElement.scrollTo({
        top: 0,
        behavior: 'smooth',
      })
      ...
      <div class="ltablet:max-w-full flex max-w-[425px] flex-col items-center gap-4 text-center md:flex-row md:text-left lg:max-w-full">
        <div>
          <BaseHeading as="h2" size="xl" weight="light" lead="tight" class="text-muted-800 dark:text-white">
            <span>New Prepayment</span>
          </BaseHeading>
          <BaseParagraph size="sm">
            <span class="text-muted-500"> Create a new  Prepayment record </span>
          </BaseParagraph>
        </div>
      </div>
