...
const VALIDATION_TEXT = {
  ...
  ITEM_RATE_MARKUP_REQUIRED: "Item Rate Markup is required.",
  ITEM_RATE_INDIVIDUAL_REQUIRED: "Item Rate Individual' is required.",
  PERCENTAGE_REQUIRED: "Percentage is required.",
  ROUNDOFF_REQUIRED: "Round off is required.",
...
const zodSchema = z
    .object({
      name: z.string(),
      type: z.string(),
      item_rate_markup: z.string(),
      item_rate_individual: z.string(),
      percentage: z.string(),
      roundoff: z.string(),
    ...
    .superRefine((data, ctx) => {
      ...
      if (!data.item_rate_markup) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message: VALIDATION_TEXT.ITEM_RATE_MARKUP_REQUIRED,
          path: ['item_rate_markup'],
        })
      }
      if (!data.item_rate_individual) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message: VALIDATION_TEXT.ITEM_RATE_INDIVIDUAL_REQUIRED,
          path: ['item_rate_individual'],
        })
      }
      if (!data.percentage) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message: VALIDATION_TEXT.PERCENTAGE_REQUIRED,
          path: ['percentage'],
        })
      }
      if (!data.roundoff) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message: VALIDATION_TEXT.ROUNDOFF_REQUIRED,
          path: ['roundoff'],
        })
      }
    })
...
const onSubmit = handleSubmit(
    async (values) => {
      success.value = false
      console.log('price-list-create-success', values)
      try {
        await new Promise((resolve, reject) => {
          if (values.name === '') {
            setTimeout(
                () => reject(new Error('Price list validation error')),
                2000,
            )
          }
        ...
        if (error.message === 'Price list validation error') {
    ...
    (error) => {
      success.value = false
      console.log('price-list-create-error', error)
      document.documentElement.scrollTo({
        top: 0,
        behavior: 'smooth',
      })
      ...
      <div class="ltablet:max-w-full flex max-w-[425px] flex-col items-center gap-4 text-center md:flex-row md:text-left lg:max-w-full">
        <div>
          ...
          <BaseParagraph size="sm">
            <span class="text-muted-500"> Create a new  Price List record </span>
          </BaseParagraph>
        </div>
      </div>
                ...
                <div class="grid grid-cols-12 gap-4">
                  <div class="ltablet:col-span-6 col-span-12 lg:col-span-6">
                    <Field v-slot="{ field, errorMessage, handleChange, handleBlur }" name="name">
                      <BaseInput label="Name" placeholder="Ex: Acme Inc." :model-value="field.value"
                  ...
                  <div class="tablet:col-span-6 col-span-12 lg:col-span-6">
                    <Field v-slot="{ field, errorMessage, handleChange, handleBlur }" name="item_rate_markup">
                      <BaseInput label="Item Rate Markup" placeholder="Ex: 12" :model-value="field.value"
                                 :error="errorMessage" :disabled="isSubmitting" type="text"
                                 @update:model-value="handleChange" @blur="handleBlur"/>
                    </Field>
                  </div>
                  <div class="tablet:col-span-6 col-span-12 lg:col-span-6">
                    <Field v-slot="{ field, errorMessage, handleChange, handleBlur }" name="item_rate_individual">
                      <BaseInput label="Item Rate Individual" placeholder="Ex: 23" :model-value="field.value"
                                 :error="errorMessage" :disabled="isSubmitting" type="text"
                                 @update:model-value="handleChange" @blur="handleBlur"/>
                    </Field>
                  </div>
                  <div class="tablet:col-span-6 col-span-12 lg:col-span-6">
                    <Field v-slot="{ field, errorMessage, handleChange, handleBlur }" name="percentage">
                      <BaseInput label="Percentage" placeholder="Ex: Business Income" :model-value="field.value"
                                 :error="errorMessage" :disabled="isSubmitting" type="text"
                                 @update:model-value="handleChange" @blur="handleBlur"/>
                    </Field>
                  </div>
                  <div class="tablet:col-span-6 col-span-12 lg:col-span-6">
                    <Field v-slot="{ field, errorMessage, handleChange, handleBlur }" name="roundoff">
                      <BaseInput label="Roundoff" placeholder="Ex: 2" :model-value="field.value"
                                 :error="errorMessage" :disabled="isSubmitting" type="text"
                                 @update:model-value="handleChange" @blur="handleBlur"/>
                    </Field>
                  </div>
