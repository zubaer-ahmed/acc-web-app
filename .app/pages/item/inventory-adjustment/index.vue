...
definePageMeta({title: 'Inventory Adjustment'})
...
const {data, pending, error, refresh} = await useFetch(
    '/api/core/item/inventory-adjustment/list',
    {
      query,
    },
)
...
<template>
  <div>
    <TairoContentWrapper>
      ...
      <template #right>
        <BaseButton color="primary" class="w-full sm:w-32" to="/item/inventory-adjustment/add">
          ...
          <div class="w-full">
             <TairoTable shape="rounded" class="border-separate">
              <template #header>
                ...
                <TairoTableHeading uppercase spaced>Mode Of Adjustment</TairoTableHeading>
                <TairoTableHeading uppercase spaced>Account UID</TairoTableHeading>
                <TairoTableHeading uppercase spaced>Reason</TairoTableHeading>
               ...
               <TairoTableRow v-if="selected.length==1">
                 ...
                 <TairoTableCell colspan="12" class="bg-success-100 text-success-700 dark:bg-success-700 dark:text-success-100 p-4">
                   <NuxtLink :to="`/item/inventory-adjustment/${selected[0]}/view`" class="rounded-lg bg-sky-500/50 px-5 py-2.5 text-sm font-medium text-white hover:bg-blue-800 focus:outline-none focus:ring-4 focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"> <span>View </span> </NuxtLink>
                 </TairoTableCell>
                 <TairoTableCell colspan="12" class="bg-success-100 text-success-700 dark:bg-success-700 dark:text-success-100 p-4">
                   <NuxtLink :to="`/item/inventory-adjustment/${selected[0]}/edit`" class="rounded-lg bg-sky-500/50 px-5 py-2.5 text-sm font-medium text-white hover:bg-blue-800 focus:outline-none focus:ring-4 focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"> <span>Edit </span> </NuxtLink>
                 </TairoTableCell>
                 </div>
               </TairoTableRow>
               <TairoTableRow v-if="selected.length>1">
             ...
             <TairoTableRow v-for="item in data?.data" :key="item.uid" class="even:bg-white-200 odd:bg-gray-200">
                ...
                <TairoTableCell spaced> {{ item.mode_of_adjustment}}</TairoTableCell>
                <TairoTableCell spaced> {{ item.account_uid}}</TairoTableCell>
                <TairoTableCell spaced> {{ item.reason}}</TairoTableCell>
