...
const onSubmit = handleSubmit(async (values) => {
  ...
  try {
    ...
    const signinData = await userStore.setSignin(uData)
    toaster.clearAll()
...
//3.1 google
const onSubmitGoogle = handleSubmit(async (values) => {
  let uData= {
  };
  ...
  try {
    ...
    const redirectData = await oAuthStore.setRedirect(uData)
