  ...
  <div class="h-screen md:flex">
    ...
    <div class="dark:bg-muted-900 flex flex-col items-center justify-between bg-white py-10 md:w-1/2">
      ...
      <form method="POST" action="" @submit.prevent="onSubmit" class="mt-6" novalidate>
        ...
        <BaseParagraph size="sm" class="text-muted-400 mb-6"> Enter your account credentials to sign in
        </BaseParagraph>
        <div class="space-y-4">
          ...
          <Field v-slot="{ field, errorMessage, handleChange, handleBlur }" name="password">
            <BaseInput :model-value="field.value" :error="errorMessage" :disabled="isSubmitting" type="password"
                       label="Password" placeholder="Password" icon="ph:lock-duotone"
                       @update:model-value="handleChange" @blur="handleBlur"/>
          </Field>
        </div>
        ...
        <div class="mt-6 flex items-center justify-between">
          <Field v-slot="{ field, errorMessage, handleChange, handleBlur }" name="trustDevice">
            <!--                <BaseCheckbox-->
            <!--                    :model-value="field.value"-->
            <!--                    :disabled="isSubmitting"-->
            <!--                    shape="rounded"-->
            <!--                    label="Trust for 30 days"-->
            <!--                    color="primary"-->
