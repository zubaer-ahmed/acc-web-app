...
const zodSchema = z
    ...
    .superRefine((data, ctx) => {
      ...
      if (!data.period) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message: VALIDATION_TEXT.PERIOD_REQUIRED,
          path: ['period'],
        })
      }
    })
...
const onSubmit = handleSubmit(
    async (values) => {
      success.value = false
      console.log('budget-category-create-success', values)
      try {
        await new Promise((resolve, reject) => {
          if (values.name === '') {
            setTimeout(
                () => reject(new Error('Budget Category validation error')),
                2000,
            )
          }
        ...
        if (error.message === 'Budget Category validation error') {
    ...
    (error) => {
      success.value = false
      console.log('budget-category-create-error', error)
      document.documentElement.scrollTo({
        top: 0,
        behavior: 'smooth',
      })
      ...
      <div class="ltablet:max-w-full flex max-w-[425px] flex-col items-center gap-4 text-center md:flex-row md:text-left lg:max-w-full">
        <div>
          ...
          <BaseParagraph size="sm">
            <span class="text-muted-500"> Create a new  Budget Category record </span>
          </BaseParagraph>
        </div>
      </div>
                ...
                <div class="grid grid-cols-12 gap-4">
                  <div class="ltablet:col-span-6 col-span-12 lg:col-span-6">
                    <Field v-slot="{ field, errorMessage, handleChange, handleBlur }" name="name">
                      <BaseInput label="Name" placeholder="Ex: Monthly." :model-value="field.value" :error="errorMessage" :disabled="isSubmitting" type="text" @update:model-value="handleChange" @blur="handleBlur" />
                    </Field>
                  </div>
                  <div class="ltablet:col-span-8 col-span-12 lg:col-span-8">
                    <Field v-slot="{ field, errorMessage, handleChange, handleBlur }" name="period">
                      <BaseListbox
                          label="Period"
                          :items="period"
                          :properties="{
