...
const zodSchema = z
    ...
    .superRefine((data, ctx) => {
      ...
      if (!data.currency_rate) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message: VALIDATION_TEXT.CURRENCY_RATE_REQUIRED,
          path: ['currency_rate'],
        })
      }
    })
...
const onSubmit = handleSubmit(
    async (values) => {
      success.value = false
      console.log('exchange-rate-create-success', values)
      try {
        await new Promise((resolve, reject) => {
          if (values.currency_id === '') {
            setTimeout(
                () => reject(new Error('Exchange rate validation error')),
                2000,
            )
          }
        ...
        if (error.message === 'Exchange rate validation error') {
          setFieldError('currency_id', 'This currency  is not allowed')
    ...
    (error) => {
      success.value = false
      console.log('exchange-rate-create-error', error)
      document.documentElement.scrollTo({
        top: 0,
        behavior: 'smooth',
      })
      ...
      <div
        ...
        <div>
          ...
          <BaseParagraph size="sm">
            <span class="text-muted-500"> Create a new  Exchange Rate  record </span>
          </BaseParagraph>
        </div>
      </div>
                ...
                <div class="grid grid-cols-12 gap-4">
                  ...
                  <div class="ltablet:col-span-6 col-span-12 lg:col-span-6">
                    <Field v-slot="{ field, errorMessage, handleChange, handleBlur }" name="currency_rate">
                      <BaseInput label="Curency Rate" placeholder="Ex: 2023-12-31" :model-value="field.value"
