...
definePageMeta({title: 'Fixed Asset NBV'})
...
const {data, pending, error, refresh} = await useFetch(
    '/api/core/account/current-asset-nbv/list',
    {
      query,
    },
)
...
<template>
  <div>
    <TairoContentWrapper>
      ...
      <template #right>
        <BaseButton color="primary" class="w-full sm:w-32" to="/account/fixed-asset-nbv/add">
          ...
          <div class="w-full">
            ...
            <TairoTableHeading uppercase spaced class="p-4">
              ...
              />
                ...
                <TairoTableHeading uppercase spaced>Remaining Life</TairoTableHeading>
                <TairoTableHeading uppercase spaced>Salvage Value</TairoTableHeading>
                <TairoTableHeading uppercase spaced>Through Date</TairoTableHeading>
                <TairoTableHeading uppercase spaced>Net Book Value</TairoTableHeading>
              </template>
             ...
             <TairoTableRow v-if="selected.length==1">
               ...
               <TairoTableCell colspan="12" class="bg-success-100 text-success-700 dark:bg-success-700 dark:text-success-100 p-4">
                 <NuxtLink :to="`/account/fixed-asset-nbv/${selected[0]}/view`" class="rounded-lg bg-sky-500/50 px-5 py-2.5 text-sm font-medium text-white hover:bg-blue-800 focus:outline-none focus:ring-4 focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"> <span>View </span> </NuxtLink>
               </TairoTableCell>
               <TairoTableCell colspan="12" class="bg-success-100 text-success-700 dark:bg-success-700 dark:text-success-100 p-4">
                 <NuxtLink :to="`/account/fixed-asset-nbv/${selected[0]}/edit`" class="rounded-lg bg-sky-500/50 px-5 py-2.5 text-sm font-medium text-white hover:bg-blue-800 focus:outline-none focus:ring-4 focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"> <span>Edit </span> </NuxtLink>
               </TairoTableCell>
               </div>
             </TairoTableRow>
              <TairoTableRow v-if="selected.length>1">
             ...
             <TairoTableRow v-for="item in data?.data" :key="item.uid" class="even:bg-white-200 odd:bg-gray-200">
                ...
                <TairoTableCell spaced> {{ item.remaining_life }}</TairoTableCell>
                <TairoTableCell spaced> {{ item.salvage_value }}</TairoTableCell>
                <TairoTableCell spaced> {{ item.through_date }}</TairoTableCell>
                <TairoTableCell spaced> {{ item.net_book_value }}</TairoTableCell>
              </TairoTableRow>
            </TairoTable>
          </div>
          <div class="mt-6">
