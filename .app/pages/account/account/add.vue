<script setup lang="ts">
import {toTypedSchema} from '@vee-validate/zod'
import {Field, useFieldError, useForm} from 'vee-validate'
import {z} from 'zod'

definePageMeta({title: 'Add'})

const accountType=[
  {
    uid: "123",
    name: 'rajib'
  },
  {
    uid: "234",
    name: 'pavel'
  },
]
const currencyCode=[
  {
    code: "USD",
    name: 'USD'
  },
  {
    code: "GBP",
    name: 'GBP'
  },
  {
    code: "BDT",
    name: 'BDT'
  },
]

const VALIDATION_TEXT = {
  NAME_REQUIRED: "Name is required.",
  CURRENCY_CODE_REQUIRED: "Currency Code is required.",
  OPENING_BALANCE_REQUIRED: "Opening Balance is required.",
  IS_ENABLED_REQUIRED: "Enabled is required.",
  ACCOUNT_CODE_REQUIRED: "Account Code is required.",
  ACCOUNT_TYPE_REQUIRED: "Account Type is required.",
  DESCRIPTION_REQUIRED: "Description is required.",
  TAX_TYPE_REQUIRED: "Tax Type is required.",
  DO_ENABLE_PAYMENTS_REQUIRED: "Enable Payment is required.",
  DO_EXPENSE_CLAIMS_REQUIRED: "Expense Claim is required.",
  DO_ADD_TO_WATCH_LIST_REQUIRED: "Add To Watch List is required.",
  REPORTING_CODE_REQUIRED: "Reporting Code is required.",
}
const zodSchema = z
    .object({
      name: z.string().min(1, VALIDATION_TEXT.NAME_REQUIRED),
      currency_code: z.string(),
      opening_balance: z.coerce.number(),
      is_enabled: z.boolean(),
      account_code: z.string(),
      account_type_uid: z.string(),
      description: z.string(),
      tax_type: z.string(),
      // do_enable_payments: z.boolean(),
      // do_expense_claims: z.boolean(),
      // do_add_to_watch_list: z.boolean(),
      // reporting_code: z.string(),

    })
    .superRefine((data, ctx) => {
      if (!data.name) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message: VALIDATION_TEXT.NAME_REQUIRED,
          path: ['name'],
        })
      }
      if (!data.account_code) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message: VALIDATION_TEXT.ACCOUNT_CODE_REQUIRED,
          path: ['account_code'],
        })
      }
      if (!data.currency_code) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message: VALIDATION_TEXT.CURRENCY_CODE_REQUIRED,
          path: ['currency_code'],
        })
      }
      if (!data.account_type_uid) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message: VALIDATION_TEXT.ACCOUNT_TYPE_REQUIRED,
          path: ['account_type_uid'],
        })
      }
    })

type FormInput = z.infer<typeof zodSchema>

const validationSchema = toTypedSchema(zodSchema)
const initialValues: any = computed<FormInput>(() => ({
  name: '',
  currency_code: '',
  opening_balance: 0.00,
  is_enabled: false,
  account_code: '',
  account_type_uid: '',
  description: '',
  tax_type: '',
  // do_enable_payments: false,
  // do_expense_claims: false,
  // do_add_to_watch_list: false,
  // reporting_code: '',
}))

const {
  handleSubmit,
  isSubmitting,
  setFieldError,
  meta,
  values,
  errors,
  resetForm,
  setFieldValue,
  setErrors,
} = useForm({
  validationSchema,
  initialValues,
})

const success = ref(false)
const fieldsWithErrors = computed(() => Object.keys(errors.value).length)
onBeforeRouteLeave(() => {
  if (meta.value.dirty) {
    return confirm('You have unsaved changes. Are you sure you want to leave?')
  }
})
const toaster = useToaster()
const onSubmit = handleSubmit(
    async (values) => {
      success.value = false
      console.log('account-create-success', values)
      try {
        await new Promise((resolve, reject) => {
          if (values.name === '') {
            setTimeout(
                () => reject(new Error('Name validation error')),
                2000,
            )
          }
          setTimeout(resolve, 4000)
        })

        toaster.clearAll()
        await toaster.show({
          title: 'Success',
          message: `Record has been created!`,
          color: 'success',
          icon: 'ph:check',
          closable: true,
        })
      } catch (error: any) {
        if (error.message === 'Name validation error') {
          setFieldError('name', 'This name can not be empty.')

          document.documentElement.scrollTo({
            top: 0,
            behavior: 'smooth',
          })

          toaster.clearAll()
          await toaster.show({
            title: 'Oops!',
            message: 'Please review the errors in the form',
            color: 'danger',
            icon: 'lucide:alert-triangle',
            closable: true,
          })
        }
        return
      }

      resetForm()

      document.documentElement.scrollTo({
        top: 0,
        behavior: 'smooth',
      })

      success.value = true
      setTimeout(() => {
        success.value = false
      }, 3000)
    },
    (error) => {
      success.value = false
      console.log('account-create-error', error)
      document.documentElement.scrollTo({
        top: 0,
        behavior: 'smooth',
      })
    },
)
</script>
<template>
  <div>
    <div class="mb-4 flex flex-col justify-between md:flex-row md:items-center">
      <div
          class="ltablet:max-w-full flex max-w-[425px] flex-col items-center gap-4 text-center md:flex-row md:text-left lg:max-w-full">
        <div>
          <BaseHeading as="h2" size="xl" weight="light" lead="tight" class="text-muted-800 dark:text-white">
            <span>New Account</span>
          </BaseHeading>
          <BaseParagraph size="sm">
            <span class="text-muted-500"> Create a new  Account record. </span>
          </BaseParagraph>
        </div>
      </div>
      <div class="mt-4 flex items-center justify-center gap-2 md:mt-0 md:justify-start">
        <BaseButtonAction @click.prevent="$router.back()">
          <Icon name="lucide:arrow-left" class="h-3 w-3"/>
          <span>Cancel</span>
        </BaseButtonAction>
      </div>
    </div>
    <BaseCard>
      <form method="POST" action="" class="divide-muted-200 dark:divide-muted-700 grid divide-x sm:grid-cols-2"
            @submit.prevent="onSubmit">
        <div shape="curved" class="bg-muted-50 dark:bg-muted-800/60 space-y-8 p-10">
          <div class="mx-auto flex w-full max-w-[410px] flex-col">
            <div>
              <div>
                <div class="grid grid-cols-12 gap-4">
                  <div class="ltablet:col-span-6 col-span-12 lg:col-span-6">
                    <Field v-slot="{ field, errorMessage, handleChange, handleBlur }" name="account_code">
                      <BaseInput label="Account Code" placeholder="Ex: IN-234" :model-value="field.value"
                                 :error="errorMessage" :disabled="isSubmitting" type="text"
                                 @update:model-value="handleChange" @blur="handleBlur"/>
                    </Field>
                  </div>
                  <div class="ltablet:col-span-8 col-span-12 lg:col-span-6">
                    <Field v-slot="{ field, errorMessage, handleChange, handleBlur }" name="name">
                      <BaseInput label="Name" placeholder="Ex: Income." :model-value="field.value"
                                 :error="errorMessage" :disabled="isSubmitting" type="text"
                                 @update:model-value="handleChange" @blur="handleBlur"/>
                    </Field>
                  </div>
                  <div class="ltablet:col-span-6 col-span-12 lg:col-span-6">
                    <Field v-slot="{ field, errorMessage, handleChange, handleBlur }" name="currency_code">
                      <BaseListbox
                          label="Currency Code"
                          :items="currencyCode"
                          :properties="{
                       value: 'code',
                       label: 'name',
                        }"
                          :model-value="field.value"
                          :error="errorMessage"
                          :disabled="isSubmitting"
                          @update:model-value="handleChange"
                          @blur="handleBlur"
                      />
                    </Field>
                  </div>

                  <div class="ltablet:col-span-6 col-span-12 lg:col-span-6">
                    <Field v-slot="{ field, errorMessage, handleChange, handleBlur }" name="account_type_uid">
                      <BaseListbox
                          label="Account Type"
                          :items="accountType"
                          :properties="{
                                        value: 'uid',
                                        label: 'name',
                                      }"
                          :model-value="field.value"
                          :error="errorMessage"
                          :disabled="isSubmitting"
                          @update:model-value="handleChange"
                          @blur="handleBlur"
                      />
                    </Field>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div shape="curved" class="w-full space-y-8 p-10">
          <div class="mx-auto w-full max-w-[410px]">
            <div class="grid grid-cols-12 gap-4">

              <div class="ltablet:col-span-6 col-span-12 lg:col-span-6">
                <Field v-slot="{ field, errorMessage, handleChange, handleBlur }" name="opening_balance">
                  <BaseInput label="Opening Balance" placeholder="Ex: 3214.00" :model-value="field.value"
                             :error="errorMessage" :disabled="isSubmitting" type="text"
                             @update:model-value="handleChange" @blur="handleBlur"/>
                </Field>
              </div>
<!--              <div class="ltablet:col-span-6 col-span-12 lg:col-span-6">-->
<!--                <Field v-slot="{ field, errorMessage, handleChange, handleBlur }" name="reporting_code">-->
<!--                  <BaseInput label="Reporting Code" placeholder="Ex: hello@acme.co" :model-value="field.value"-->
<!--                             :error="errorMessage" :disabled="isSubmitting" type="text"-->
<!--                             @update:model-value="handleChange" @blur="handleBlur"/>-->
<!--                </Field>-->
<!--              </div>-->
              <div class="ltablet:col-span-6 col-span-12 lg:col-span-6">
                <Field v-slot="{ field, errorMessage, handleChange, handleBlur }" name="tax_type">
                  <BaseInput label="Tax Type" placeholder="Ex: INPUT" :model-value="field.value"
                             :error="errorMessage" :disabled="isSubmitting" type="text"
                             @update:model-value="handleChange" @blur="handleBlur"/>
                </Field>
              </div>
<!--              <div class="ltablet:col-span-6 col-span-12 lg:col-span-6">-->
<!--                <Field v-slot="{ field, errorMessage, handleChange, handleBlur }" name="do_enable_payments">-->
<!--                  <BaseCheckbox label="Enable Payments" shape="rounded" color="primary"-->
<!--                                :model-value="field.value" :error="errorMessage" :disabled="isSubmitting"-->
<!--                                @update:model-value="handleChange" @blur="handleBlur"/>-->
<!--                </Field>-->
<!--              </div>-->
<!--              <div class="ltablet:col-span-6 col-span-12 lg:col-span-6">-->
<!--                <Field v-slot="{ field, errorMessage, handleChange, handleBlur }" name="do_enable_payments">-->
<!--                  <BaseCheckbox label="Expense Claims" shape="rounded" color="primary"-->
<!--                                :model-value="field.value" :error="errorMessage" :disabled="isSubmitting"-->
<!--                                @update:model-value="handleChange" @blur="handleBlur"/>-->
<!--                </Field>-->
<!--              </div>-->

<!--              <div class="ltablet:col-span-6 col-span-12 lg:col-span-6">-->
<!--                <Field v-slot="{ field, errorMessage, handleChange, handleBlur }" name="do_enable_payments">-->
<!--                  <BaseCheckbox label="Add To Watch List" shape="rounded" color="primary"-->
<!--                                :model-value="field.value" :error="errorMessage" :disabled="isSubmitting"-->
<!--                                @update:model-value="handleChange" @blur="handleBlur"/>-->
<!--                </Field>-->
<!--              </div>-->

              <div class="ltablet:col-span-6 col-span-12 lg:col-span-12">
                <Field v-slot="{ field, errorMessage, handleChange, handleBlur }" name="description">
                  <BaseInput label="Description" placeholder="Ex: Business Income" :model-value="field.value"
                             :error="errorMessage" :disabled="isSubmitting" type="text"
                             @update:model-value="handleChange" @blur="handleBlur"/>
                </Field>
              </div>
              <div class="ltablet:col-span-6 col-span-12 lg:col-span-6">
                <Field v-slot="{ field, errorMessage, handleChange, handleBlur }" name="is_enabled">
                  <BaseCheckbox label="Enable for Transaction" shape="rounded" color="primary"
                                :model-value="field.value" :error="errorMessage" :disabled="isSubmitting"
                                @update:model-value="handleChange" @blur="handleBlur"/>
                </Field>
              </div>
            </div>
            <div class="mt-5 flex flex-col-reverse text-right md:block md:space-x-3">
              <BaseButton type="submit" color="primary" class="!h-12 w-full sm:w-40"> Create</BaseButton>
            </div>
          </div>
        </div>
      </form>
    </BaseCard>
  </div>
</template>
