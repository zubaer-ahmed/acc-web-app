...
const VALIDATION_TEXT = {
  ...
  LOCKING_DATE_REQUIRED: "Locking Date is required.",
}
const zodSchema = z
    .object({
      account_type: z.string(),
      locking_date: z.string(),
    })
    .superRefine((data, ctx) => {
      ...
      if (!data.locking_date) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message: VALIDATION_TEXT.LOCKING_DATE_REQUIRED,
          path: ['locking_date'],
        })
      }
    })
...
const onSubmit = handleSubmit(
    async (values) => {
      success.value = false
      console.log('account-type-locking-create-success', values)
      try {
        await new Promise((resolve, reject) => {
          if (values.account_type === '') {
            setTimeout(
                () => reject(new Error('Account Type validation error')),
                2000,
            )
          }
        ...
        if (error.message === 'Account Type validation error') {
          setFieldError('account_type', 'This account type can not be empty.')
    ...
    (error) => {
      success.value = false
      console.log('account-type-locking-create-error', error)
      document.documentElement.scrollTo({
        top: 0,
        behavior: 'smooth',
      })
      ...
      <div
        ...
        <div>
          <BaseHeading as="h2" size="xl" weight="light" lead="tight" class="text-muted-800 dark:text-white">
            <span>New Account Type Locking</span>
          </BaseHeading>
          <BaseParagraph size="sm">
            <span class="text-muted-500"> Create a new  Account Type Locking record </span>
          </BaseParagraph>
        </div>
      </div>
            ...
            <div>
              <div>
                <div class="grid grid-cols-12 gap-4">
                  ...
                  <div class="ltablet:col-span-6 col-span-12 lg:col-span-6">
                    <Field v-slot="{ field, errorMessage, handleChange, handleBlur }" name="locking_date">
                      <BaseInput label="Locking Date" placeholder="Ex: 2023-12-31" :model-value="field.value"
