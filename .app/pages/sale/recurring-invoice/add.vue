...
const onSubmit = handleSubmit(
    async (values) => {
      success.value = false
      console.log('recurring-invoice-create-success', values)
      try {
        await new Promise((resolve, reject) => {
          if (values.contact_uid === '') {
            setTimeout(
                () => reject(new Error('Recurring invoice validation error')),
                2000,
            )
          }
        ...
        if (error.message === 'Recurring invoice validation error') {
          setFieldError('contact_uid', 'This invoice is not allowed')
    ...
    (error) => {
      success.value = false
      console.log('recurring-invoice-create-error', error)
      document.documentElement.scrollTo({
        top: 0,
        behavior: 'smooth',
      })
      ...
      <div class="ltablet:max-w-full flex max-w-[425px] flex-col items-center gap-4 text-center md:flex-row md:text-left lg:max-w-full">
        <div>
          ...
          <BaseParagraph size="sm">
            <span class="text-muted-500"> Create a new  Recurring Invoice record </span>
          </BaseParagraph>
        </div>
      </div>
            ...
            <div class="grid grid-cols-12 gap-4">
              ...
              <div class="ltablet:col-span-6 col-span-12 lg:col-span-12">
                <Field v-slot="{ field, errorMessage, handleChange, handleBlur }" name="ends_at">
                  <BaseInput label="Ends At" placeholder="Ex: 2023-01-01" :model-value="field.value"
                             :error="errorMessage" :disabled="isSubmitting" type="text"
                             @update:model-value="handleChange" @blur="handleBlur"/>
                </Field>
              </div>
