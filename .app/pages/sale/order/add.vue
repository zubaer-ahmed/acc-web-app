...
const VALIDATION_TEXT = {
  ...
  ATTENTION_TO_REQUIRED: "Attention To is required.",
  TELEPHONE_REQUIRED: "Telephone is required.",
  DELIVERY_INSTRUCTIONS_REQUIRED: "Delivery Instructions is required.",
  EXPECTED_ARRIVAL_DATE_REQUIRED: "Expected Arrival Date is required.",
...
const zodSchema = z
    ...
    .superRefine((data, ctx) => {
      ...
      if (!data.delivery_address) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message: VALIDATION_TEXT.DELIVERY_ADDRESS_REQUIRED,
      ...
      if (!data.expected_arrival_date) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message: VALIDATION_TEXT.EXPECTED_ARRIVAL_DATE_REQUIRED,
      ...
      try {
        await new Promise((resolve, reject) => {
          if (values.contact_uid === '') {
            setTimeout(
                () => reject(new Error('Order validation error')),
                2000,
            )
          }
        ...
        if (error.message === 'Order validation error') {
          setFieldError('contact_uid', 'This order is not allowed')
      ...
      <div class="ltablet:max-w-full flex max-w-[425px] flex-col items-center gap-4 text-center md:flex-row md:text-left lg:max-w-full">
        <div>
          ...
          <BaseParagraph size="sm">
            <span class="text-muted-500"> Create a new  Order record </span>
          </BaseParagraph>
        </div>
      </div>
            ...
            <div>
              <div>
                <div class="grid grid-cols-12 gap-4">
                  ...
                  <div class="ltablet:col-span-6 col-span-12 lg:col-span-6">
                    <Field v-slot="{ field, errorMessage, handleChange, handleBlur }" name="delivery_address">
                      <BaseInput label="delivery Address" placeholder=" " :model-value="field.value"
                                 :error="errorMessage" :disabled="isSubmitting" type="text"
                                 @update:model-value="handleChange" @blur="handleBlur"/>
                    </Field>
                  </div>
                  <div class="ltablet:col-span-6 col-span-12 lg:col-span-6">
                    <Field v-slot="{ field, errorMessage, handleChange, handleBlur }" name="expected_arrival_date">
                      <BaseInput label="Expected Arrival Date" placeholder="Ex: 2023-01-12" :model-value="field.value"
                                 :error="errorMessage" :disabled="isSubmitting" type="text"
                                 @update:model-value="handleChange" @blur="handleBlur"/>
                    </Field>
                  </div>
        ...
        <div shape="curved" class="w-full space-y-8 p-10">
          <div class="mx-auto w-full max-w-[410px]">
            <div class="grid grid-cols-12 gap-4">
              ...
              <div class="ltablet:col-span-6 col-span-12 lg:col-span-6">
                <Field v-slot="{ field, errorMessage, handleChange, handleBlur }" name="attention_to">
                  <BaseInput label="Attention to" placeholder=" " :model-value="field.value"
                             :error="errorMessage" :disabled="isSubmitting" type="text"
                             @update:model-value="handleChange" @blur="handleBlur"/>
                </Field>
              </div>
              <div class="ltablet:col-span-6 col-span-12 lg:col-span-6">
                <Field v-slot="{ field, errorMessage, handleChange, handleBlur }" name="telephone">
                  <BaseInput label="Telephone" placeholder="Ex: +9838888234" :model-value="field.value"
                             :error="errorMessage" :disabled="isSubmitting" type="text"
                             @update:model-value="handleChange" @blur="handleBlur"/>
                </Field>
              </div>
              <div class="ltablet:col-span-6 col-span-12 lg:col-span-6">
                <Field v-slot="{ field, errorMessage, handleChange, handleBlur }" name="delivery_instructions">
                  <BaseInput label="Delivery Instructions" placeholder=" " :model-value="field.value"
                             :error="errorMessage" :disabled="isSubmitting" type="text"
                             @update:model-value="handleChange" @blur="handleBlur"/>
                </Field>
              </div>
