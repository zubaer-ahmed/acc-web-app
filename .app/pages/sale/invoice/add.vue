...
const zodSchema = z
    ...
    .superRefine((data, ctx) => {
      ...
      if (!data.invoice_date) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message: VALIDATION_TEXT.INVOICE_DATE_REQUIRED,
          path: ['invoice_date'],
        })
      }
...
const onSubmit = handleSubmit(
    async (values) => {
      success.value = false
      console.log('invoice-create-success', values)
      try {
        await new Promise((resolve, reject) => {
          if (values.contact_uid === '') {
            setTimeout(
                () => reject(new Error('Invoice validation error')),
                2000,
            )
          }
        ...
        if (error.message === 'Invoice validation error') {
      ...
      <div class="ltablet:max-w-full flex max-w-[425px] flex-col items-center gap-4 text-center md:flex-row md:text-left lg:max-w-full">
        <div>
          ...
          <BaseParagraph size="sm">
            <span class="text-muted-500"> Create a new  Invoice record </span>
          </BaseParagraph>
        </div>
      </div>
            ...
            <div>
              <div>
                <div class="grid grid-cols-12 gap-4">
                  ...
                  <div class="ltablet:col-span-6 col-span-12 lg:col-span-6">
                    <Field v-slot="{ field, errorMessage, handleChange, handleBlur }" name="invoice_date">
                      <BaseInput label="Invoice Date" placeholder="Ex: 2023-12-01" :model-value="field.value"
                                 :error="errorMessage" :disabled="isSubmitting" type="text"
                                 @update:model-value="handleChange" @blur="handleBlur"/>
                    </Field>
                  </div>
        ...
        <div shape="curved" class="w-full space-y-8 p-10">
          <div class="mx-auto w-full max-w-[410px]">
            <div class="grid grid-cols-12 gap-4">
              ...
              <div class="ltablet:col-span-6 col-span-12 lg:col-span-6">
                <Field v-slot="{ field, errorMessage, handleChange, handleBlur }" name="due_date">
                  <BaseInput label="Due Date" placeholder="Ex: 2023-12-01" :model-value="field.value"
                             :error="errorMessage" :disabled="isSubmitting" type="text"
                             @update:model-value="handleChange" @blur="handleBlur"/>
                </Field>
              </div>
