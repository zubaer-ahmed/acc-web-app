...
import {useCurrentAsset} from '@/composables/core/account/current-asset';
...
export const useCurrentAssetStore = defineStore('current-asset', {
  state: () => ({
  ...
  actions: {
    ...
    async setReinstate(payload: any) {
      const {reinstate} = useCurrentAsset();
