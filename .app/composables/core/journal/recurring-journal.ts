...
export function useRecurringJournal() {

    const config = useRuntimeConfig();
    const baseURL=config.public.coreApi;

    async function list(payload: any){
        const endpoint=baseURL+'/recurring-journal/list';
    ...
    async function read(payload: any){
        const endpoint=baseURL+'/recurring-journal/read';
        const { isFetching, error, data } = await useFetch(endpoint).post(payload).json();
        if(!error.value){
            if(data.value && data.value.length!=0){
                const status=data.value.data.original.status;
    ...
    async function add(payload: any){
        const endpoint=baseURL+'/recurring-journal/add';
        const { isFetching, error, data } = await useFetch(endpoint).post(payload).json();
    ...
    async function edit(payload: any){
        const endpoint=baseURL+'/recurring-journal/edit';
        const { isFetching, error, data } = await useFetch(endpoint).post(payload).json();
        if(!error.value){
            if(data.value && data.value.length!=0){
                const status=data.value.data.original.status;
    ...
    async function remove(payload: any){
        const endpoint=baseURL+'/recurring-journal/remove';
        const { isFetching, error, data } = await useFetch(endpoint).post(payload).json();
        if(!error.value){
            if(data.value && data.value.length!=0){
                const status=data.value.data.original.status;
    ...
    async function reinstate(payload: any){
        const endpoint=baseURL+'/recurring-journal/reinstate';
        const { isFetching, error, data } = await useFetch(endpoint).post(payload).json();
        if(!error.value){
            if(data.value && data.value.length!=0){
                const status=data.value.data.original.status;
    ...
    async function vital(payload: any){
        const endpoint=baseURL+'/recurring-journal/vital';
        const { isFetching, error, data } = await useFetch(endpoint).post(payload).json();
        if(!error.value){
            if(data.value && data.value.length!=0){
                const status=data.value.data.original.status;
    ...
    async function load(payload: any){
        const endpoint=baseURL+'/recurring-journal/import';
        const { isFetching, error, data } = await useFetch(endpoint).post(payload).json();
        if(!error.value){
            if(data.value && data.value.length!=0){
                const status=data.value.data.original.status;
    ...
    async function unload(payload: any){
        const endpoint=baseURL+'/recurring-journal/export';
        const { isFetching, error, data } = await useFetch(endpoint).post(payload).json();
        if(!error.value){
            if(data.value && data.value.length!=0){
                const status=data.value.data.original.status;
